...
public class TheRealMCTSPacman extends MCTree<Game, MOVE>{
	...
	private int maxIterations = 286;
	private int maxDPolicyIters = 4;
	
	private double levelCompleteReward = 300;
	private double pillEatenReward = 0.07577648859322661;
	private double eatenGhostReward = 93.74496135480338;
	private double pacmanWasEatenReward = -67.0989187122142;
	private double noGhostsEatenAfterPowerPillReward = -58.861444436148645;
	private double distaRectionRewardMult = 40.76931277791056;
	// DistaRection will only be taken into account when the ghosts get closer to the Trigger
	private double distaRectionTrigger = 74.98732143167571;
	private double ghostDistanceAfterPPThreshold = 84.48668643413174;
	private double ghostDistanceAfterPPReward = 32.07708608906784;
	...
	public TheRealMCTSPacman(double[] params) {
		...
		this.ghostDistanceAfterPPReward = params[9];
	}
	...
	private double rewardPowerPillGrab(Game state){
		...
		for(GHOST ghost : GHOST.values()){
			...
			if(distanceFromPacman < ghostDistanceAfterPPThreshold){
				double notNormalizedReward = (ghostDistanceAfterPPThreshold - distanceFromPacman);
				double normalizedReward = (notNormalizedReward - 0) / (ghostDistanceAfterPPThreshold - 0);
				reward += normalizedReward * ghostDistanceAfterPPReward;
			}
		}
		return reward;
	}
...
//	}
	...
	private double getReward(Game prevState, Game finalState){
		...
		if(prevDistaRection < distaRectionTrigger){
			...
			//Normalizing distarection
			double normalizedDistaRectionDelta = (distaRectionDelta - 1) / (distaRectionTrigger - 1);
			if(areGhostsEdible(finalState)==false){
				reward += normalizedDistaRectionDelta * distaRectionRewardMult;
			}
		}
